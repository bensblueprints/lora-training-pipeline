...
FACE_DIR = "/home/benji/ComfyUI/input/cast_faces/"
OUT_BASE = "/home/benji/ComfyUI/output/lora_kontext/"
SELS = "/home/benji/selections.json"
CAST_GAMES = "/home/benji/cast_games.json"
STEPS = 12
GUIDANCE = 2.5
CLIENT_ID = "batch_fb_5060"
...
def load_games():
    global SLUG_TO_GAME
    try:
        SLUG_TO_GAME = json.load(open(CAST_GAMES)).get("slug_to_game", {})
    except Exception:
        pass

# ---- FIX: 20% composite face (identity 0.69 + natural head) + 832x1248 tall canvas ----
FULL_W, FULL_H = 832, 1248
HEAD_W, HEAD_H = 1024, 1024
FULL_INDICES = [0, 6, 7, 9, 10, 13, 14, 15]   # only regenerate these full-body poses
FACE_FRAC = 0.20

def make_fullbody_ref(face_path, out_path):
    face = Image.open(face_path).convert("RGB")
    W, H = FULL_W, FULL_H
    face_w = int(W * FACE_FRAC)
...
BIKER_CHARS = [
    "Roxanne \"Roxie\" Vance", "Ray \"Rayzor\" Martinez", "Dale \"Diesel\" Kowalski",
    "Junior Vance", "Cheryl Vance", "Wes Bulle", "Bird Tubbs", "Sonny Kiel",
    "Akane Matsuda", "Alita Kyubey",
]

def build_workflow(image_name, prompt, seed, prefix, W=1024, H=1024):
    return {
        "1": {"class_type": "UNETLoader", "inputs": {"unet_name": "flux1-kontext-dev-fp8-e4m3fn.safetensors", "weight_dtype": "fp8_e4m3fn"}},
...
def queue(wf):
    return post(COMFY + "/prompt", {"prompt": wf, "client_id": CLIENT_ID})["prompt_id"]
    ...
    while time.time() - t0 < timeout:
        ...
        if pid in h:
            ...
            if st.get("status_str") == "error":
                raise RuntimeError("ComfyUI error: " + json.dumps(h[pid].get("messages", []))[:400])
            if st.get("completed"):
                return h[pid]
        time.sleep(2)
...
def main():
    ...
    for ci, char_name in enumerate(ordered):
        ...
        for i in FULL_INDICES:
            ...
            prompt_final = PROMPTS[i].replace("{OUTFIT}", outfit)
            t0 = time.time()
            try:
                wf = build_workflow(full_ref_name, prompt_final, 42 + i, f"lora_kontext/{slug}/{i:02d}", FULL_W, FULL_H)
                pid = queue(wf)
                h = wait_done(pid)
                ...
                out = imgs[0]
                src = os.path.join("/home/benji/ComfyUI/output", out["subfolder"], out["filename"])
                shutil.copy(src, out_path)
                dt = time.time() - t0
                total += 1
                print(f"  [{ci+1}/{len(ordered)}] {char_name}: fullbody {i:02d}/20 ({dt:.0f}s)", flush=True)
            except Exception as e:
                print(f"  FAIL {char_name} #{i}: {e}", flush=True)
                time.sleep(3)
    print(f"\nDONE: {total} full-body images @ {FACE_FRAC} composite", flush=True)
